...
// ─── Noise SVG di-cache sebagai konstanta, tidak di-parse ulang tiap render ──
...
// ─── WordReveal dimemo agar tidak re-render jika props sama ──────────────────
const WordReveal = memo(({ text, className, delay = 0 }) => {
  ...
  return (
    ...
    >
      {words.map((word, i) => (
        <motion.span variants={child} key={i} className="mr-[0.2em] pb-1 inline-block">
          {word}
        </motion.span>
      ))}
    </motion.div>
...
function IntroScreen({ onNext }) {
  return (
    ...
    >
      {/* ── Background layers — pointer-events-none agar tidak block tap ── */}
      <div
        className="absolute inset-0 z-0 opacity-[0.15] pointer-events-none mix-blend-multiply"
        style={{ backgroundImage: NOISE_BG }}
      />
      <div
        className="absolute inset-0 z-0 opacity-[0.05] pointer-events-none"
        style={{ backgroundImage: GRID_BG, backgroundSize: '64px 64px' }}
      />

      {/* Watermark — animasi hanya sekali (once), tidak loop */}
      <motion.div
      ...
      >
        <span className="flex items-center gap-1">
          <Zap size={10} className="text-yellow-500" /> POWERED BY MEMORIES
        </span>
