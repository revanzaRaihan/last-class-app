      ...
      <div className="flex items-center gap-2">
        ...
        <div>
          <h3 className="font-bold text-xs md:text-sm text-slate-900 leading-none mb-0.5 flex items-center gap-1">
            ...
            {(msg.is_anon || msg.isAnon) && (
              <span className="text-[8px] font-mono bg-gray-100 text-gray-500 px-1 rounded">ANON</span>
            )}
          </h3>
...
const MarqueeRow = ({ items, direction = 'left', speed = 40, delay = 0 }) => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-y-0 left-0 w-8 md:w-16 bg-gradient-to-r from-[#f8f9fa] to-transparent z-10 pointer-events-none" />
...
const MenfesModal = ({ onClose, onSubmit }) => {
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      name:    mode === 'anon' ? 'Anonymous' : initialText.trim().toUpperCase().slice(0, 10),
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-gray-100">
          ...
          <button onClick={onClose} className="w-7 h-7 rounded-full border border-gray-200 flex items-center justify-center hover:bg-gray-100 transition-colors">
          ...
          <AnimatePresence mode="wait">
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  {[
                    ...
                    { key: 'initial', icon: User,  label: 'Pakai Inisial', sub: 'misal: A.S / RDY' },
                    ...
                    >
                      <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${mode === key ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-500'}`}>
                        <Icon size={20} />
                      </div>
                ...
                <AnimatePresence>
                  {mode === 'initial' && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden mb-4">
                      <label className="text-[10px] font-mono font-bold text-gray-500 uppercase tracking-wider block mb-1.5">Inisial / Nama Panggilan</label>
                      <input type="text" maxLength={10} value={initialText} onChange={e => setInitialText(e.target.value)}
                        placeholder="contoh: RDY atau A.S"
                        className="w-full border-2 border-black rounded-lg px-3 py-2 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-blue-500 placeholder:font-normal placeholder:text-gray-300 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]" />
                    </motion.div>
                  )}
                </AnimatePresence>
                ...
                <button onClick={handleNext}
                  ...
                  className="w-full bg-black text-white py-2.5 rounded-xl font-bold text-sm disabled:opacity-40 disabled:cursor-not-allowed hover:bg-blue-600 transition-colors flex items-center justify-center gap-2">
                  Lanjutkan →
                </button>
              </motion.div>
            )}
            ...
            {step === 2 && (
              ...
              >
                ...
                <label className="text-[10px] font-mono font-bold text-gray-500 uppercase tracking-wider block mb-1.5">Pesan kamu</label>
                <textarea autoFocus value={message} onChange={e => { setMessage(e.target.value); setError(null) }}
                  maxLength={200} rows={4} placeholder="Tulis pesanmu untuk semua... 💙"
                  className="w-full border-2 border-black rounded-xl px-3 py-2.5 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-blue-500 placeholder:text-gray-300 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] mb-1" />
                <div className="flex justify-between items-center mb-4">
                  <AnimatePresence>
                    {error && (
                      <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                        className="text-[10px] text-red-500 font-mono flex items-center gap-1">
                        <AlertCircle size={10} /> {error}
                      </motion.p>
                    )}
                  </AnimatePresence>
                  <span className="text-[9px] font-mono text-gray-400 ml-auto">{message.length}/200</span>
                </div>
  ...
  return (
    <section className="relative h-[100dvh] w-full bg-[#f8f9fa] flex flex-col justify-between overflow-hidden text-slate-900 font-sans">
      ...
      <div className="flex-1 flex flex-col items-center justify-center text-center px-4 relative z-20">
        ...
        >
          <UserX size={9} /> bisa anonim • tersimpan permanen di database
        </motion.p>
      </div>
      ...
      <AnimatePresence>
        {showModal && <MenfesModal onClose={() => setShowModal(false)} onSubmit={handleSubmit} />}
      </AnimatePresence>
